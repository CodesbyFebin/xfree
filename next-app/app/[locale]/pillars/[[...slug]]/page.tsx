...
export async function generateStaticParams() {
  ...
  LOCALES.forEach(locale => {
    ...
    PILLARS.forEach(pillar => {
      ...
      params.push({ locale, slug: [pillar.category, pillar.slug] });
    });
  });
  return params;
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (slug.length === 1) {
    ...
    if (pillar) {
      const canonical = buildCanonical(`/pillars/${slug[0]}`, { language: locale });
      const allKeywords = [...(pillar.keywords || []), 'XFree', 'pillar', 'tool hub', pillar.category].filter(Boolean);
      return { title: `XFree ${pillar.name} | Free Tool Hub`, description: pillar.description, keywords: allKeywords, alternates: { canonical }, openGraph: { title: `XFree ${pillar.name}`, description: pillar.description, url: canonical, type: 'article' }, twitter: { card: 'summary_large_image', title: `XFree ${pillar.name}`, description: pillar.description } };
    }
  }
  if (slug.length === 2) {
    ...
    if (pillar && pillar.category === category) {
      const canonical = buildCanonical(`/pillars/${category}/${pillarSlug}`, { language: locale });
      const allKeywords = [...(pillar.keywords || []), 'XFree', pillar.category].filter(Boolean);
      return { title: `XFree ${pillar.name} | ${pillar.toolCount} Free Tools`, description: pillar.description, keywords: allKeywords, alternates: { canonical }, openGraph: { title: `XFree ${pillar.name}`, description: pillar.description, url: canonical, type: 'article' }, twitter: { card: 'summary_large_image', title: `XFree ${pillar.name}`, description: pillar.description } };
    }
  }
...
export default async function PillarPage({ params }: Props) {
  ...
  if (slug.length === 1) { const pillar = findPillarBySlug(slug[0]); if (pillar) return <PillarDetail pillar={pillar} dict={dict} locale={currentLocale} />; }
  if (slug.length === 2) { const [category, pillarSlug] = slug; const pillar = findPillarBySlug(pillarSlug); if (pillar && pillar.category === category) return <PillarDetail pillar={pillar} dict={dict} locale={currentLocale} />; }
  notFound();
}

function PillarsIndex({ dict, locale }: { dict: any; locale: Locale }) {
  const breadcrumbItems = [{ name: 'Pillars', href: `/${locale}/pillars` }];
  return (
    <>
      ...
      <main id="main-content" className="pt-20">
        <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <header className="text-center mb-12"><h1 className="text-3xl sm:text-4xl font-black text-white font-mono mb-4"><span className="text-cyber-glow">$</span> XFree Tool Pillars</h1><p className="text-cyber-muted max-w-2xl mx-auto">{PILLARS.length} organized tool hubs covering JSON data tools, regex patterns, SEO utilities, security tools, and more.</p></header>
          <div className="space-y-10">
            {PILLAR_CATEGORIES.map(cat => {
              ...
              return (
                <section key={cat.id}>
                  <h2 className="text-xl font-bold text-white font-mono mb-4 flex items-center gap-2"><span className="text-2xl">{cat.icon}</span>{cat.label}<span className="text-xs text-cyber-dim font-normal">({categoryPillars.length} hubs)</span></h2>
...
function PillarDetail({ pillar, dict, locale }: { pillar: NonNullable<ReturnType<typeof findPillarBySlug>>; dict: any; locale: Locale }) {
  ...
  const breadcrumbItems = [{ name: 'Home', href: `/${locale}` }, { name: 'Pillars', href: `/${locale}/pillars` }, { name: categoryInfo?.label || pillar.category, href: `/${locale}/pillars` }, { name: pillar.name, href: `/${locale}/pillars/${pillar.slug}` }];
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <header className="mb-8">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div><h1 className="text-2xl sm:text-3xl font-bold text-white font-mono">XFree {pillar.name}</h1><p className="text-cyber-muted mt-1">{categoryInfo?.label} • {pillar.toolCount || pillarTools.length} tools</p></div>
            </div>
          ...
          {pillarTools.length > 0 ? (
            <section className="mb-12"><h2 className="text-xl font-bold text-white font-mono mb-6"><span className="text-cyber-glow">$</span> Tools in this Pillar</h2>
            ...
            <section className="mb-12 cyber-card p-8 text-center"><p className="text-cyber-muted font-mono">More tools coming soon for this pillar.</p></section>
          )}
          {categoryPillars.length > 1 && (
            <section className="mb-12"><h2 className="text-xl font-bold text-white font-mono mb-6"><span className="text-cyber-glow">$</span> Related {categoryInfo?.label} Pillars</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {categoryPillars.filter(p => p.slug !== pillar.slug).slice(0, 6).map(related => (
                  <Link key={related.slug} href={`/${locale}/pillars/${related.slug}`} className="cyber-card p-4 group block">
          ...
          <section className="mb-12"><h2 className="text-xl font-bold text-white font-mono mb-6"><span className="text-cyber-glow">$</span> Why Use XFree {pillar.name}?</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="cyber-card p-5"><h3 className="text-sm font-semibold text-white font-mono mb-2">🔒 Privacy-First</h3><p className="text-xs text-cyber-muted">All tools in this pillar run 100% in your browser. Your data never leaves your device.</p></div>
              ...
              <div className="cyber-card p-5"><h3 className="text-sm font-semibold text-white font-mono mb-2">🔗 Interconnected</h3><p className="text-xs text-cyber-muted">Tools in this pillar link to related tools for seamless workflows and better results.</p></div>
            </div>
          </section>
        </div>
      </main>
