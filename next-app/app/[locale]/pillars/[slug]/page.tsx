import { Metadata } from 'next';
import { LOCALES, LOCALE_METADATA, type Locale } from '@/lib/i18n';
import { PILLARS, findPillarBySlug } from '@/lib/data/pillars';
import { buildCanonical } from '@/lib/canonical';
import { notFound } from 'next/navigation';

export async function generateStaticParams() {
  return PILLARS.map((pillar) => ({ slug: pillar.slug }));
}

export async function generateMetadata({
  params,
}: {
  params: Promise<{ locale: Locale; slug: string }>;
}): Promise<Metadata> {
  const { locale, slug } = await params;
  const pillar = findPillarBySlug(slug);
  if (!pillar) return {};
  const canonical = buildCanonical(`/pillars/${slug}`, { language: locale });
  return {
    metadataBase: new URL('https://www.xfree.in'),
    title: `${pillar.name} — XFree`,
    description: pillar.description,
    alternates: {
      canonical,
      languages: Object.fromEntries(
        LOCALES.map((loc) => [loc, buildCanonical(`/pillars/${slug}`, { language: loc })])
      ),
    },
  };
}

export default function PillarDetailPage({
  params,
}: {
  params: Promise<{ locale: Locale; slug: string }>;
}) {
  const resolvedParams = params instanceof Promise ? params : Promise.resolve(params);
  
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="cyber-card p-8">
        <div className="text-3xl mb-4">🧩</div>
        <h1 className="text-3xl font-black text-white mb-4">Pillar Detail</h1>
        <p className="text-cyber-muted">Pillar detail pages coming soon.</p>
      </div>
    </div>
  );
}
