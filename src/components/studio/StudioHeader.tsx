...
export function StudioHeader({ mode, onModeChange }: { mode: ProcessingMode; onModeChange: (mode: ProcessingMode) => void }) {
  return <header className="flex h-14 shrink-0 items-center justify-between border-b border-stone-200 bg-white/90 px-4 backdrop-blur-md lg:px-6">
    <a href="https://www.xfree.in/" className="flex items-center gap-3" aria-label="XFree homepage">
      ...
      <span><h1 className="text-[15px] font-semibold leading-tight text-stone-900">XFree Agent Studio</h1><small className="hidden text-[10px] text-stone-400 sm:block">Local-first command center</small></span>
    </a>
    <div className="flex items-center gap-3">
      ...
      <button className="flex h-8 w-8 items-center justify-center rounded-lg text-stone-400 hover:bg-stone-100" aria-label="Studio settings" title="Agent SOUL settings are available in the Local Agent panel"><Settings className="h-4 w-4" /></button>
    </div>
  </header>;
}
